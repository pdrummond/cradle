...
cradle.Connection.prototype.rawRequest = function (method, path, options, data, headers) {
    ...
    // Set Cookie Auth
    if(this.cookieAuth) {
	headers['Cookie'] = this.cookieAuth.cookieToken;
    }
...
// Connection.login()
//
// Login for Cookie based auth.
//
cradle.Connection.prototype.login = function (username, password, callback) {
    var that = this;
    var method = "POST";
    var path = "/_session";
    var data = "name=" + encodeURIComponent(username) + "&password=" + encodeURIComponent(password);
    var headers = {host: this.host, 
		   "Content-Type": "application/x-www-form-urlencoded",
		   "X-CouchDB-WWW-Authenticate": "Cookie"};

    var request = this.rawRequest(method, path, null, data, headers);
    ...
    request.on('response', function (res) {
        ...
        res.on('data', function (chunk) {
		    ...
		    that.cookieAuth.cookieToken = res.headers['set-cookie'][0];
                    // If the `raw` option was set, we return the parsed
                    // body as-is. If not, we wrap it in a `Response` object.
                    callback(null, that.options.raw ? json : new(cradle.Response)(json, res));
                }
...
// Connection.logout()
//
// Logout for Cookie based auth.
//
cradle.Connection.prototype.logout = function (callback) {
    var that = this;
    var method = "DELETE";
    var path = "/_session";
    var headers = {host: this.host, 
    ...
    var request = this.rawRequest(method, path, null, null, headers);
    ...
    request.on('response', function (res) {
        ...
        res.on('data', function (chunk) {
		    ...
		    that.cookieAuth.cookieToken = new Object();
                    // If the `raw` option was set, we return the parsed
                    // body as-is. If not, we wrap it in a `Response` object.
                    callback(null, that.options.raw ? json : new(cradle.Response)(json, res));
                }
